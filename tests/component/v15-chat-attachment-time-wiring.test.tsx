...
beforeAll(async () => {
  ...
  vi.doMock("../../src/infrastructure/current-page-sync", () => ({
    createCurrentPageSyncBridge: () => ({
      sync: vi.fn(async () => ({ tabId: 1, video })),
    }),
  }));
...
describe("v15 附件时间来源（切片 4 用户症状：恒显示 0s）", () => {
  it("播放器轮询读到的时间进入附件 currentTimeMs 并显示为紧凑文本", async () => {
  ...
  it("播放器读取失败后自动恢复轮询，附件时间不再恒 0s（自愈）", async () => {
    readTime.mockRejectedValue(new Error("player unavailable"));
    // 等待真实轮询 interval（750ms）触发一次失败并进入 block。
    await new Promise((resolve) => setTimeout(resolve, 1_200));
    expect(readTime).toHaveBeenCalled();

    // 播放器恢复；block 超时（00:05）后应自动重试并恢复采样。
    readTime.mockResolvedValue(65_000);
    await new Promise((resolve) => setTimeout(resolve, 6_200));

    const file = new File(["fixture"], "photo2.png", { type: "image/png" });
    const chat = latestShellProps?.chat;
    expect(chat).toBeDefined();
    const result = chat!.onAttachImages?.([file]);
    if (result && typeof (result as Promise<unknown>).then === "function") {
